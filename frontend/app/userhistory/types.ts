...
export interface BackendTransaction {
  ...
  user?: unknown;
  warnings?: string[] | Record<string, unknown>; // Add warnings field to handle JsonNode
}
...
export function transformTransactionToHistoryItem(
  ...
  if (transaction.warnings) {
    if (Array.isArray(transaction.warnings)) {
      ...
      typeof transaction.warnings === "object" &&
      transaction.warnings !== null
    ) {
      // Handle JsonNode case - extract array from JSON structure
      const warningObj = transaction.warnings as Record<string, unknown>;
      warnings = Array.isArray(warningObj) ? (warningObj as string[]) : [];
    }
  }
